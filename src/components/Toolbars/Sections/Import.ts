...
export default (components: OBC.Components) => {
  ...
  loadBtn.tooltipText = "Loads an IFC file into the scene. The IFC gets automatically converted to Fragments."
  ...
  async function loadTiles() {
    ...
    input.addEventListener("change", async () => {
      ...
      for (const file of input.files) {
        const { name } = file
        const key = name.replace('-processed.json', '').replace('-processed-properties.json', '');
        if (!grouped[key]) {
